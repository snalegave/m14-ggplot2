...
diamonds.sample <- diamonds %>% sample_n(size = 1000)
...
ggplot(diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, size = price))

# Try coloring the above plot based on the diamond's price!
ggplot(diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, color = price))
...
ggplot(diamonds.sample) + geom_smooth(mapping = aes(x = carat, y = price))
